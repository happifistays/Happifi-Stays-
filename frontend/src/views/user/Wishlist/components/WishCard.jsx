  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/api/v1/shops/stats`, {
          headers: {
            ...
            <CardBody className="py-md-2 d-flex flex-column h-100">
              <div className="d-flex justify-content-between align-items-center">
                  ...
                  {Array.from({ length: 5 }).map((_, index) => (
                    <li key={index} className="list-inline-item me-1 small">
                      <FaStar
                        size={16}
                        className={
                          index < Number(wishCard?.totalReviews)
                            ? "text-warning"
                            : "text-secondary"
                        }
                      />
                ...
                <span className="small text-muted">
                  {wishCard?.totalReviews > 0
                    ? `(${wishCard?.totalReviews} reviews)`
                    : "No reviews yet"}
                </span>
                <ul className="list-inline mb-0 items-center gap-1">
                  ...
                  <Dropdown className="list-inline-item">
                    <DropdownToggle
                      as={Link}
                      to=""
                      className="arrow-none btn btn-sm btn-round btn-light mb-0"
                    >
                      <FaShareAlt size={10} />
                    </DropdownToggle>
                    <DropdownMenu
              ...
              <h5 className="card-title mb-1">
                <Link to={`/hotels/detail/${wishCard?._id}`}>
                  {wishCard?.listingName}
                </Link>
              </h5>
              <small className="items-center">
                <BsGeoAlt className=" me-2" />
                {wishCard?.location?.street}, {wishCard?.location?.city},{" "}
                {wishCard?.location?.state}, {wishCard?.location?.country} -{" "}
                {wishCard?.location?.postalCode}
              </small>
